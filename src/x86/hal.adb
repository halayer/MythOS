...
package body HAL is
    ...
    procedure init is
        ...
        features : CPUID.CPUID_ret := CPUID.CPUID(CPUID.Features);
        caches : CPUID.CPUID_ret := CPUID.CPUID(CPUID.Caches);
    begin
        ...
        if edx_features_val.APIC then PIC.toggle(False); end if;
    end init;
    ...
    procedure halt is begin
        System.Machine_Code.Asm("halt.loop: cli; hlt; jmp halt.loop",
                                Volatile => True);
    end halt;
    ...
    package body Interrupts is
        ...
        procedure set_handler(intr: Number; ir: Handler) is
            ...
            function ir_to_u32 is new Ada.Unchecked_Conversion(Handler, u32);
            
            addr : u32 := ir_to_u32(ir);
        begin
            IDT.IDT(intr).target_lo  := u16(addr and 16#FFFF#);
            IDT.IDT(intr).target_hi  := u16(shr(addr, 16));
